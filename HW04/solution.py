...
def FindBestMatches(descriptors1, descriptors2, threshold):
    ...
    for  i in range(y1):
        for j in range(y2):
            # 두 descriptor 사이의 angle을 구함, 이때 descriptor는 normalized되어 있어서 크기가 1임
            # 따라서 내적을 한 후 (a*b*cos(theta), a=b=1) inverse cosin을 해주면 angle이 나온다
            # angle이 작을수로 더욱더 match가 됨 
            temp[j] = math.acos(np.dot(descriptors1[i], descriptors2[j]))
        
        # angle이 작은 순으로 정렬하는데 이때 angle 값이 아니라 index값이 들어오게됨
        compare = sorted(range(len(temp)), key = lambda k: temp[k])

        # ratio distance 사용, ratio distance가 threshold값보다 작아야됨
...
def KeypointProjection(xy_points, h):
    ...
    # START
    hc_xys = np.pad(xy_points, pad_width=((0,0), (0,1)), mode='constant', constant_value=1)
    xys_p = h @ hc_xys.T
...
def RANSACHomography(xy_src, xy_ref, num_iter, tol):
    ...
    for i in range(num_iter):
        ...
        H = cv2.findHomomgraphy(src_sample, ref_sample)# 이거 cv2 안쓰고 하기
        xy_res = KeypointProjection(xy_src, H[0])
        res_dis = np.sqrt(np.sum((xy_res - xy_ref)**2, axis=1))
        inliner = np.sum(res_dis < tol)

        if inliner > max_inliers:
            h = H[0]
            max_inliers = inliner
    # END
    assert isinstance(h, np.ndarray)
